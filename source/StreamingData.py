...
def get_label(row):
    if row['Label0-pass'] == 1 :
        return 0
    elif row['Label0-fail'] == 1:
        return 1
    elif row['Label1-pass'] == 1:
        return 2
    elif row['Label1-fail'] == 1:
        return 3
    elif row['Label2-pass'] == 1:
        return 4
    elif row['Label2-fail'] == 1:
        return 5
...
def iterabel_dataset_generation(path):
    ...
    df['new_label'] = None
    # 应用映射并更新新标签列
    for col, new_val in label_mapping.items():
        mask = df[col] == 1
        df.loc[mask, 'new_label'] = new_val
    # 保留不包含“Label”的列和新的“new_label”列
    non_label_cols = [col for col in df.columns if 'Label' not in col]
    df = df[non_label_cols]
    # 删除包含None值的行
    df = df[df['new_label'].notna()]
